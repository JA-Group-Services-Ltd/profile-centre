...
function loadGtag(measurementId: string): void {
  ...
  // - anonymize_ip: strips last IP octet before sending to Google
  // - allow_google_signals: off — no cross-site tracking or remarketing
  // - allow_ad_personalization_signals: off — no ad profiling
  // - restricted_data_processing: on — Google cannot use data for its own purposes
...
export default function GoogleAnalytics() {
  ...
  // Sanitise the path: strip numeric/UUID segments so no user IDs leak
  // e.g. /dashboard/profile/42 → /dashboard/profile/[id]
  useEffect(() => {
    if (!consentedRef.current || typeof window.gtag !== 'function') return;
    const sanitised = location.pathname
      .replace(/\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi, '/[id]')
      .replace(/\/\d+/g, '/[id]');
    window.gtag('event', 'page_view', { page_path: sanitised });
  }, [location.pathname]);
