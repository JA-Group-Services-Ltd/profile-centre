...
export async function notifyWelcome(opts: {
  ...
  safe(async () => {
    await sendEmail({ fromName: 'Sousa Murray Profiles', to: opts.userEmail, subject, html, text, replyTo: EMAIL_REPLY_TO });
  });
}
...
export async function notifyNewMessage(opts: {
  ...
  safe(async () => {
    await sendEmail({ fromName: 'Sousa Murray Profiles', to, subject, html, text, replyTo: EMAIL_REPLY_TO });
  });
}
...
export async function notifySupportRequest(opts: {
  ...
  safe(async () => {
    await sendEmail({ fromName: 'Sousa Murray Profiles', to, subject, html, text, replyTo: opts.userEmail });
  });
}
...
export async function notifyEnquiryReceived(opts: {
  ...
  safe(async () => {
    await sendEmail({
      fromName: 'Sousa Murray Profiles',
      to: opts.ownerEmail,
      subject,
      html,
      text,
...
export async function notifyEnquiryConfirmation(opts: {
  ...
  safe(async () => {
    await sendEmail({ fromName: 'Sousa Murray Profiles', to: opts.senderEmail, subject, html, text, replyTo: EMAIL_REPLY_TO });
  });
}
