 ...
 *
 * Generates a comprehensive PDF manual for JA Profile Studio.
...
function newPage(ctx: Ctx): PDFPage {
  ...
  page.drawText(s('JA Profile Studio - Platform Manual'), { x: MARGIN, y: A4H - 28, size: 7, font: ctx.fontR, color: C.muted });
...
function buildCover(ctx: Ctx, section: string) {
  ...
  page.drawText('JA Profile Studio', { x: MARGIN, y: titleY, size: 32, font: ctx.fontB, color: C.white });
...
function buildAdminManual(ctx: Ctx) {
  ...
  drawPara(ctx, 'The JA Profile Studio Admin Portal is accessible at /admin. It is protected by two layers of authentication: JA Group Services ID (Microsoft Entra OIDC) and a secondary PIN gate. Only authorised JA Group Services staff may access the admin portal.');
  ...
  drawPara(ctx, 'JA Profile Studio supports 14 profile types, each with type-specific sections:');
  const profileTypes = [
  ...
  drawTableRow(ctx, ['4', 'JA Branded', 'JA Profile Studio brand colours with blue header'], [80, 200, 200], false);
  ctx.y -= 8;
  ...
  drawPara(ctx, 'Wordmark: "Created with JA Profile Studio" footer appears on Starter plan posters. Removed automatically on Professional, Organisation, Ultimate Organisation, Ultimate Organisation+, and Lifetime plans.');
...
function buildUserManual(ctx: Ctx) {
  ...
  drawPara(ctx, 'JA Profile Studio is a digital business card and professional profile platform. This guide covers everything you need to know to set up and manage your account.');
  drawNote(ctx, 'JA Profile Studio is available to UK-based users aged 18 and over. You authenticate via JA Group Services ID — no separate password is needed.', 'info');
  ...
  drawBullet(ctx, 'Template 4 — JA Branded: JA Profile Studio brand colours with blue header');
  drawPara(ctx, 'Each poster includes: name, job title, company, bio, skills, email, phone, website, and a scannable QR code. On Starter plans a "Created with JA Profile Studio" wordmark appears; this is removed on Professional, Business, and Lifetime plans.');
  ...
  drawPara(ctx, 'Build a branded email signature that links to your JA Profile Studio. Available on Starter and higher plans.');
  ...
  drawNote(ctx, 'The signature includes a link to your live JA Profile Studio. Keep your profile published for the link to work.', 'tip');
  ...
  drawPara(ctx, 'Exercise your UK GDPR rights directly from your dashboard. All requests are reviewed by the JA Profile Studio team and responded to within 30 days.');
...
export async function getManualPdf(req: Request, res: Response) {
  ...
  doc.setTitle('JA Profile Studio - Platform Manual');
  ...
  doc.setCreator('JA Profile Studio - japrofilestudio.jagroupservices.co.uk');
  doc.setKeywords(['JA Profile Studio', 'manual', 'admin', 'user guide', 'JA Group Services']);
  ...
  if (section === 'user' || section === 'all') {
    if (section === 'all') {
      ...
      p.drawText('For JA Profile Studio users', { x: MARGIN, y: A4H / 2 - 36, size: 12, font: fontI, color: C.gold });
    }
    buildUserManual(ctx);
  }
