...
function collectSarData(userId: number) {
  ...
  return {
    _meta: {
      ...
      note: 'This document contains all personal data held by JA Profile Studio under UK GDPR. It is provided in response to a Subject Access Request.',
    },
    account: user,
    consent,
    subscriptions,
    ...
    legacy: {
      _note: 'These sections relate to features that have been removed from JA Profile Studio. Data is included for completeness under UK GDPR but these features are no longer active.',
  ...
  try {
    ...
    function drawChrome() {
      ...
      pdfDoc.getPages().forEach((pg, idx) => {
        ...
        pg.drawText('JA Profile Studio - Subject Access Request', {
    ...
    page.drawText('JA Profile Studio - japrofilestudio.jagroupservices.co.uk', { x: ML, y: PAGE_H - 125, size: 9, font: fontR, color: rgb(0.8, 0.9, 1.0) });
    page.drawText(s(`Prepared for: ${fmt(d.account.name)} <${fmt(d.account.email)}>`), { x: ML, y: PAGE_H - 148, size: 9, font: fontR, color: C_WHITE });
    page.drawText(s(`JA Profile Studio User Number: ${d.account.user_number ? String(d.account.user_number).replace(/(\d{3})(\d{3})(\d{3})(\d{3})/, '$1 $2 $3 $4') : 'N/A'}`), { x: ML, y: PAGE_H - 162, size: 9, font: fontR, color: C_WHITE });
    ...
    y -= 14;
    page.drawText('This document contains personal data processed by JA Profile Studio under UK GDPR.', { x: ML, y, size: 8, font: fontR, color: C_GRAY });
    y -= 12;
    ...
    const stampRows: [string, string][] = [
      ...
      ['JA Profile Studio User Number', d.account.user_number ? String(d.account.user_number).replace(/(\d{3})(\d{3})(\d{3})(\d{3})/, '$1 $2 $3 $4') : 'N/A'],
    ...
    kv('JA Profile Studio User Number', d.account.user_number ? String(d.account.user_number).replace(/(\d{3})(\d{3})(\d{3})(\d{3})/, '$1 $2 $3 $4') : 'N/A', true);
    ...
    infoBox('The following sections relate to features that have been removed from JA Profile Studio.');
