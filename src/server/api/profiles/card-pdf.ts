...
// ── Template 4: JA Branded (Sousa Murray Profiles brand colours) ───────────────────
const template4: TemplateFn = async (pdfDoc, page, data, fontB, fontR) => {
  ...
  // Sousa Murray Profiles wordmark in header
  page.drawText('Sousa Murray Profiles', { x: BLEED + 12, y: BLEED + CARD_H - 22, size: 10, font: fontB, color: white });
...
export async function profileCardPdf(req: Request, res: Response) {
  ...
  pdfDoc.setAuthor('Sousa Murray Profiles');
  pdfDoc.setSubject('Print-ready business card');
  pdfDoc.setCreator('Sousa Murray Profiles — japrofilestudio.jagroupservices.co.uk');
  ...
  // Instructions header
  sheetPage.drawText('Sousa Murray Profiles — Print-Ready Business Card', {
  ...
  sheetPage.drawText('4. For best results, use 350gsm silk or gloss card stock. 5. QR code links to your live Sousa Murray Profiles.', {
    x: 50, y: instrY - 26, size: 7.5, font: fontR, color: hexToRgb('#6b7280'),
  });

  // Footer
  sheetPage.drawText(`Generated by Sousa Murray Profiles · japrofilestudio.jagroupservices.co.uk · ${new Date().toLocaleDateString('en-GB', { timeZone: 'Europe/London' })}`, {
