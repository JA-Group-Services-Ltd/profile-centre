...
// ── Template 4: JA Branded (JA Profile Studio brand colours) ───────────────────
const template4: TemplateFn = async (pdfDoc, page, data, fontB, fontR) => {
  ...
  // JA Profile Studio wordmark in header
  page.drawText('JA Profile Studio', { x: BLEED + 12, y: BLEED + CARD_H - 22, size: 10, font: fontB, color: white });
...
export async function profileCardPdf(req: Request, res: Response) {
  ...
  // Instructions header
  sheetPage.drawText('JA Profile Studio — Print-Ready Business Card', {
  ...
  sheetPage.drawText('4. For best results, use 350gsm silk or gloss card stock. 5. QR code links to your live JA Profile Studio.', {
    x: 50, y: instrY - 26, size: 7.5, font: fontR, color: hexToRgb('#6b7280'),
  });

  // Footer
  sheetPage.drawText(`Generated by JA Profile Studio · japrofilestudio.jagroupservices.co.uk · ${new Date().toLocaleDateString('en-GB', { timeZone: 'Europe/London' })}`, {
